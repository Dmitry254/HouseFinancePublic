...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.add_img = tk.PhotoImage(file='D:\\HouseFinance\\images\\add.png')
        btn_open_dialog = tk.Button(toolbar, text='Добавить позицию',
                                    command=lambda: self.open_dialog(),
                                    bg='#d7dBe0', bd=0, compound=tk.TOP,
                                    image=self.add_img)
        btn_open_dialog.pack(side=tk.LEFT)

        self.update_img = tk.PhotoImage(file='D:\\HouseFinance\\images\\edit.png')
        btn_edit_dialog = tk.Button(toolbar, text='Редактировать', bg='#d7dBe0', bd=0,
                                    image=self.update_img, compound=tk.TOP, command=lambda: self.open_update_dialog())
        btn_edit_dialog.pack(side=tk.LEFT)

        self.delete_img = tk.PhotoImage(file='D:\\HouseFinance\\images\\delete.png')
        btn_delete = tk.Button(toolbar, text='Удалить', bg='#d7dBe0', bd=0,
                                        image=self.delete_img, compound=tk.TOP, command=lambda: self.delete_records())
        btn_delete.pack(side=tk.LEFT)

        self.tree = ttk.Treeview(self, columns=('ID', 'description', 'costs', 'total'),
                                 heigh=15, show='headings')
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_descripion = ttk.Label(self, text='Наименование:')
        label_descripion.place(x=50, y=50)
        label_select = ttk.Label(self, text='Доход или расход')
